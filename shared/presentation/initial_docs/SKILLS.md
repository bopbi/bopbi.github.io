...
4. Submit - saved with `pending` status

## Approve/Reject Content (Admin Workflow)

1. Admin reviews pending content in `/admin/komunitas` or `/admin/panduan`
2. Click "Setujui" to approve or "Tolak" to reject
3. Content status updates: `pending` → `approved` or `rejected`
4. Approved content appears publicly
